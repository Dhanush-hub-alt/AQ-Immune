...
export const getAQIColor = (aqi: number) => {
  ...
  if (aqi <= 300) return '#9333ea'; // Purple
...
export const generateHealthRecommendations = (aqi: number) => {
  ...
  if (aqi <= 50) {
    ...
    recommendations.push(
      'Members of sensitive groups should limit outdoor exertion.'
    );
    ...
    recommendations.push(
      'Consider staying indoors or using air purifiers.'
    );
  } else if (aqi <= 300) {
...
export const filterByConditions = <T extends Record<string, any>>(
  arr: T[],
