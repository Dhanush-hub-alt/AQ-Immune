...
export async function loadLSTMModel(url: string) {
  const model = await tf.loadLayersModel(url);
  return model;
}

// series: number[] (historical values)
export async function predictNextSteps(model: tf.LayersModel, series: number[], steps = 6) {
  ...
  for (let i = 0; i < steps; i++) {
    // predict next
    // @ts-ignore
    const out = model.predict(lastInput) as tf.Tensor;
    const v = (await out.data())[0];
    preds.push(v * std + mean);

    // shift input window
    const arr = await lastInput.array() as number[][][];
