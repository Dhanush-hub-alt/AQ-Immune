...
export interface PredictionResult {
    shortTerm: PredictionPoint[]; // 6-hour forecast
    longTerm: PredictionPoint[];  // 24-hour forecast
...
const noise = (scale: number = 1): number =>
    (Math.random() - 0.5) * 2 * scale;

// Trend detection: positive = worsening, negative = improving
const detectTrend = (data: number[]): number => {
...
export const runLSTMPrediction = (
    recentAQI: number[],
    recentPM25: number[],
    recentCO2: number[]
): PredictionResult => {
    ...
    const shortTerm: PredictionPoint[] = [];
    let runAQI = lastAQI;
    ...
    const longTerm: PredictionPoint[] = [];
    runAQI = lastAQI;
    runPM25 = lastPM25;
    runCO2 = lastCO2;
    ...
    for (let h = 1; h <= 24; h++) {
        ...
        const nightMultiplier =
            futureHour >= 0 && futureHour <= 5 ? 0.88 : 1.0;
