      ...
      <div className="flex items-center justify-between px-4 lg:px-6 h-16">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={alertRef}>
            ...
            {/* Alert Dropdown */}
            {showAlerts && (
              <div className="absolute right-0 mt-2 w-80 glass-card rounded-2xl border border-white/10 overflow-hidden shadow-2xl z-50 animate-fade-in-up">
                <div className="flex items-center justify-between p-4 border-b border-white/5">
                  <h3 className="font-semibold text-white text-sm">Recent Alerts</h3>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-500">{sampleAlerts.filter(a => !a.read).length} unread</span>
                    <button onClick={() => setShowAlerts(false)} className="text-slate-400 hover:text-white">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                </div>
                <div className="divide-y divide-white/5 max-h-72 overflow-y-auto">
                  {sampleAlerts.map((alert) => (
                    <div key={alert.id} className={`p-3 flex items-start gap-3 ${!alert.read ? 'bg-white/5' : ''}`}>
                      <div className={`mt-0.5 p-1.5 rounded-lg border ${severityColor(alert.severity)}`}>
                        <AlertTriangle className="w-3 h-3" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-white/90 font-medium leading-snug">{alert.message}</p>
            ...
            <div className="relative" ref={profileRef}>
              <button
                ...
                className="flex items-center gap-2.5 pl-2 pr-3 py-1.5 rounded-xl
                  hover:bg-white/10 transition-all border border-transparent hover:border-white/10"
              >
                ...
                <div className="hidden md:block text-left">
                  <p className="text-xs font-semibold text-white leading-none">{user.displayName?.split(' ')[0] || 'User'}</p>
              ...
              {showProfile && (
                <div className="absolute right-0 mt-2 w-52 glass-card rounded-2xl border border-white/10 overflow-hidden shadow-2xl z-50 animate-fade-in-up">
