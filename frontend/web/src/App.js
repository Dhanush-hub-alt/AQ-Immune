...
const MainLayout = ({ children }) => {
    ...
    return (_jsxs("div", { className: "flex h-screen w-full overflow-hidden bg-slate-50 dark:bg-slate-950 transition-colors duration-300", children: [_jsx(Sidebar, { open: sidebarOpen, onClose: () => setSidebarOpen(false) }), _jsxs("div", { className: "flex-1 flex flex-col h-full overflow-hidden lg:ml-64", children: [_jsx(Navbar, { onMenuClick: () => setSidebarOpen(!sidebarOpen) }), _jsx("main", { className: "flex-1 overflow-y-auto w-full h-full relative", children: children })] })] }));
};
// ── App ──────────────────────────────────────────────────
export const App = () => {
    const isAuthenticated = useAppSelector(selectIsAuthenticated);
